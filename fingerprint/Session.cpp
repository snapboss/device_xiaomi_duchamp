...
ndk::ScopedAStatus Session::generateChallenge() {
    ...
    mWorker->schedule(Callable::from([this] { mEngine->generateChallengeImpl(mCb.get()); }));
...
ndk::ScopedAStatus Session::revokeChallenge(int64_t challenge) {
    ...
    mWorker->schedule(Callable::from(
...
ndk::ScopedAStatus Session::enumerateEnrollments() {
    ...
    mWorker->schedule(Callable::from([this] { mEngine->enumerateEnrollmentsImpl(mCb.get()); }));
...
ndk::ScopedAStatus Session::getAuthenticatorId() {
    ...
    mWorker->schedule(Callable::from([this] { mEngine->getAuthenticatorIdImpl(mCb.get()); }));
...
ndk::ScopedAStatus Session::resetLockout(const keymaster::HardwareAuthToken& hat) {
    ...
    mWorker->schedule(Callable::from([this, hat] { mEngine->resetLockoutImpl(mCb.get(), hat); }));
...
ndk::ScopedAStatus Session::onPointerUp(int32_t pointerId) {
    ...
    mWorker->schedule(Callable::from([this, pointerId] { mEngine->onPointerUpImpl(pointerId); }));
    return ndk::ScopedAStatus::ok();
}

ndk::ScopedAStatus Session::onUiReady() {
    LOG(INFO) << "onUiReady";
    mWorker->schedule(Callable::from([this] { mEngine->onUiReadyImpl(); }));
    return ndk::ScopedAStatus::ok();
}
...
void Session::notify(const fingerprint_msg_t* msg) {
    ...
    switch (msg->type) {
        ...
        case FINGERPRINT_TEMPLATE_ENROLLING: {
            ...
            mCb->onEnrollmentProgress(msg->data.enroll.fid, msg->data.enroll.samples_remaining);
        } break;
